  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (mode === "signup") {
        ...
        const requiresVerification = role === "buyer";
        const { data, error: signUpError } = await supabase.auth.signUp({
          email, password,
          options: { data: { full_name: fullName, role }, ...(requiresVerification ? { emailRedirectTo: `${window.location.origin}/verify-email` } : {}) },
        });
